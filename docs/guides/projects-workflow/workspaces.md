...
- **No silent environment reuse.** Each task run has an intended environment, resolved from the issue's explicit override first, then the agent's default, then the project policy, then the instance default. When a run reuses a workspace whose environment doesn't match its intended one, Paperclip treats that as a conflict to surface rather than silently inheriting whatever the previous occupant left behind. An environment you set explicitly on an issue always wins.
